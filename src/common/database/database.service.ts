...
import uniqueKeys from "../config/unique-keys.json";
...
function quoteIdent(name: string) {
  ...
  if (
    RESERVED_TABLES.has(cleaned.toLowerCase()) ||
    RESERVED_COLS.has(cleaned.toLowerCase())
  ) {
...
function quoteReservedTables(sql: string) {
  return sql.replace(
...
function quoteDottedReserved(sql: string) {
  return sql.replace(
...
function convertMysqlFunctions(sql: string) {
  let out = sql;
...
function splitSqlAndStrings(sql: string) {
  const parts: Array<{ type: "sql" | "ph"; value?: string }> = [];
  let buf = "";
  let inSingle = false;
  let inDouble = false;
...
function flattenParams(sql: string, params: any) {
...
function toNumberedPlaceholders(sql: string) {
  const parts = splitSqlAndStrings(sql);
  let n = 1;
  let out = "";
  for (const part of parts) {
...
function objectInsert(tableSql: string, data: Record<string, any>) {
...
function conflictTarget(table: string, insertCols: string[]) {
  const keys = (uniqueKeys as Record<string, string[][]>)[table] || [];
  const colsLower = (insertCols || []).map((c) => c.toLowerCase());
  let chosen = keys.find((k) =>
    k.every((c) => colsLower.includes(String(c).toLowerCase())),
  );
...
function mysqlOnDupToConflict(sql: string, insertCols: string[]) {
...
function ensureReturning(sql: string) {
...
function wrapResult(pgResult: QueryResult) {
  const command = pgResult.command;
  const rows: any = pgResult.rows || [];
  if (command === "SELECT" || command === "WITH") {
    return rows;
  }
  const out: any = rows.slice();
  const first = rows[0] || {};
  const idKey = Object.keys(first).find((k) =>
    /^(id|.*_id|.*_ID|event_ID|Form_ID|task_ID|sticky_ID|transfer_id|comment_ID)$/i.test(
      k,
    ),
  );
...
export class DatabaseService implements OnModuleInit, OnModuleDestroy {
  ...
  private async runPrepared(client: PoolClient, sql: string, params: any[]) {
  ...
  async execute(sql: string, params?: any) {
    ...
    try {
      ...
      let updateObject: Record<string, any> | null = null;
      if (
        dupUpdateObj &&
        Array.isArray(rawParams) &&
        rawParams[1] &&
        typeof rawParams[1] === "object"
      ) {
        updateObject = rawParams[1];
        rawSql = rawSql.replace(
          /ON\s+DUPLICATE\s+KEY\s+UPDATE\s+\?/i,
          "ON DUPLICATE KEY UPDATE __OBJ__",
        );
      ...
      return await this.runPrepared(client, flattened.sql, flattened.params);
    } finally {
      client.release();
    }
  }

  query(sql: string, params?: any, cb?: (err: any, result: any) => void) {
